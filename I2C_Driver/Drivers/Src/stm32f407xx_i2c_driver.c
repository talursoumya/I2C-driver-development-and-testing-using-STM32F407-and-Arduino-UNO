...
{
	//Interrupt handling for both master and slave mode of  adevice
	...
	//note: SB flags is only applicable in master mode
	if(temp1 && temp3)
	{
		//SB flag is set
	...
	//Note:Stop detection flag is applicable only for slave mode.For master this flag will
	if(temp1 && temp3)
	{
			//STOP flag is set
		//clear the stop flag
	...
	if(temp1 && temp2 && temp3)
	{
		...
		{
			//TXE flag is set
			//WE have to do data transmission,only if application is busy in tx
			if(pI2CHandle->TxRxState == I2C_BUSY_IN_TX)
			{
...
void I2C_CloseReceiveData(I2C_Handle_t *pI2CHandle)
{
	...
	pI2CHandle->pI2Cx->CR2 &= ~(1<< I2C_CR2_ITBUFEN);
	//Implement the code to disable ITTEVFEN Control Bit
	pI2CHandle->pI2Cx->CR2 &= ~(1<< I2C_CR2_ITEVTEN);
